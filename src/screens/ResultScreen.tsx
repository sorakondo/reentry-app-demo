...
export default function ResultScreen({ judgement, onConsultExpert, onRestart }: ResultScreenProps) {
  const { t } = useLanguage();
  const resultMeta = {
  ...
  const canConsult = judgement.result === 'expert_review' || judgement.result === 'hold';

  return (
    <div className="flex flex-1 flex-col px-5 pb-6 pt-2">
      <section className={`mb-4 rounded-2xl border-2 p-5 text-center ${meta.barClass}`}>
        ...
        <section className="mb-5">
          ...
          <ul className="space-y-2">
            ...
            {judgement.dangerReasonKeys.filter((key) => key !== 'gasAlarm').map((key) => (
              <li key={key} className="rounded-xl border border-red-200 bg-red-50/60 p-3 text-base text-red-700">
        ...
        <section className="mb-5">
          ...
          <ul className="space-y-2">
            {judgement.targetedItems.map((item) => (
              <li key={item.questionId} className="rounded-xl border border-blue-200 bg-blue-50/60 p-3 text-base text-blue-800">
                {t.questions[item.questionId].shortLabel}{t.result.targetedItemSuffix}
              </li>
            ))}
          </ul>
        </section>
        ...
        <section className="mb-5">
          ...
          <ul className="space-y-2">
            {judgement.unknownItems.map((item) => (
              <li key={item.questionId} className="rounded-xl border border-amber-200 bg-amber-50/60 p-3 text-base text-amber-800">
                {t.questions[item.questionId].shortLabel}{t.result.unknownItemSuffix}
              </li>
            ))}
          </ul>
        </section>
      ...
      <div className="mt-auto space-y-3 pt-4">
        {canConsult && <BigButton onClick={onConsultExpert} className="text-lg">{t.result.consultExpertButton}</BigButton>}
