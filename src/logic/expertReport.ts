...
export function buildSavedResultNote(judgement: JudgementDetail, lang: Lang): string {
  ...
  const count = judgement.unknownItems.length;
  return lang === 'en'
    ? `Result: "${title}". ${count} item(s) could not be judged safe on site.`
    : `判定結果：「${title}」。現場で判断できなかった項目が${count}件あります。`;
}
